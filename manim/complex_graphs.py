...
class TangentGraph(Scene):
    def construct(self):
        ...
        graph = axes.plot(
            lambda x: math.tan(x), x_range=[-PI / 2 + 0.1, PI / 2 - 0.1], color=RED
        )
    ...
    def construct(self):
        ...
        graph = always_redraw(
            lambda: axes.plot_polar_curve(
                lambda theta: theta / PI, theta_range=[0, 4 * PI], color=ORANGE
            )
        )
...
class ParametricLissajous(Scene):
    def construct(self):
        ...
        graph = axes.plot_parametric_curve(
            lambda t: np.array([math.sin(3 * t), math.sin(4 * t), 0]),
            t_range=[0, 2 * PI],
            color=PURPLE,
        )
        self.play(Create(axes), Create(graph))
